...
case3_seq=open('D:\\SYSU课程\大三\毕业论文相关附件\\ml-latest-small\\version1_case3_seq.csv','w',encoding='latin1')
optimize_hit_num=open('D:\\SYSU课程\大三\毕业论文相关附件\\ml-latest-small\\version1_optimize_hit_num.csv','w',encoding='latin1')
...
def Clustering(user,moviedict): #Denpeak
    ...
    dc=DisVec[int(np.round(len(DisVec)*0.09))] #全连接，总共n*(n-1)/2个连接，DisVec总大小就是这个，选取一个参数，能控制领域数量在一定范围
...
def Judgement(cluster_dict,clustercen_index,user): 
    ...
    #找到最小簇的元素数量
    min_element_num=int(np.round(len(cluster_dict)*0.09))
...
def MovieRecommendation(userdict,moviedict):
    ...
    for u in userdict.values():
        ...
        ratio,max_cluster,case=Judgement(cluster_dict,clustercen_index,u) 
